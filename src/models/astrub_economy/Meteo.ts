...
class Meteo {
    ...
    description: string;
    effects: MeteoEffect[];
    allowedMonths?: number[]; // 1-12 (Janvier = 1, Décembre = 12)

    constructor(name: string, description: string, effects: MeteoEffect[], allowedMonths?: number[]) {
        this.name = name;
    ...
    isAvailableFor(date: Date): boolean {
        if (!this.allowedMonths || this.allowedMonths.length === 0) {
            return true;
        }
        const month = date.getMonth() + 1;
        return this.allowedMonths.includes(month);
    }
...
enum MeteosEnum {
    ...
    GEL_BRUMAIRE = "❄️ Gel de Brumaire",
    IRE_DJAUL = "👿 Ire de Djaul"
}
...
const Meteos: Record<MeteosEnum, Meteo> = {
    ...
    [MeteosEnum.ULGRUDE]: new Meteo(MeteosEnum.ULGRUDE, "Les sautes d'humeur du Protecteur de Martalo sont aussi imprévisibles que dangereuses. Nul n'est à l'abri d'un coup de foudre.", [{
        ...
        description: "", job: JobEnum.MINEUR, value: -20
    }, {
        description: "", job: JobEnum.PAYSAN, value: -20
    },], [3]),
    [MeteosEnum.REVERDIE_SILOUATE]: new Meteo(MeteosEnum.REVERDIE_SILOUATE, "Le souffle dégivrant du Protecteur d'Aperirel se mêle aux vents réchauffés et donne un coup de fouet bienfaisant à ceux qui œuvrent à l'air libre.", [{
        description: "", job: JobEnum.PAYSAN, value: 20
    }, {
        description: "", job: JobEnum.ALCHIMISTE, value: 20
    }, {
        ...
        description: "", job: JobEnum.MINEUR, value: 0
    }, {
        description: "", job: JobEnum.BUCHERON, value: 20
    },], [4]),
    [MeteosEnum.CROISSANCE_ROSAL]: new Meteo(MeteosEnum.CROISSANCE_ROSAL, "L'influence du Protecteur de Maisial invite au réveil la faune et la flore, pour le plus grand bonheur de certains récolteurs.", [{
        description: "", job: JobEnum.ALCHIMISTE, value: 50
    }, {
        description: "", job: JobEnum.PECHEUR, value: 50
    }, {
        description: "", job: JobEnum.BUCHERON, value: 15
    }, {
        description: "", job: JobEnum.MINEUR, value: 0
    }, {
        description: "", job: JobEnum.PAYSAN, value: 0
    },], [5]),
    [MeteosEnum.OPULENCE_SUMENS]: new Meteo(MeteosEnum.OPULENCE_SUMENS, "Sous le patronnage poussif du Protecteur de Juinssidor, les mineurs parviennent à murmurer à l'oreille de leurs filons.", [{
        description: "",
        job: JobEnum.MINEUR,
        value: 50
    },], [6]),
    [MeteosEnum.VOYEURISME_HECATE]: new Meteo(MeteosEnum.VOYEURISME_HECATE, "Quiconque s'aventure hors de chez soi aujourd'hui a la désagréable impression qu'on l'épie.", [{
        description: "",
        job: JobEnum.PECHEUR,
        value: -5
    }, {description: "", job: JobEnum.BUCHERON, value: -5}, {
        description: "",
        job: JobEnum.ALCHIMISTE,
        value: -5
    }, {description: "", job: JobEnum.MINEUR, value: -5}, {description: "", job: JobEnum.PAYSAN, value: -5}], [7]),
    [MeteosEnum.MATURATION_POUCHECOT]: new Meteo(MeteosEnum.MATURATION_POUCHECOT, "Le Protecteur de Fraouctor aurait été aperçu en train de se promener dans les champs. Comment expliquer, sinon, le développement soudain des plantes y poussant ?", [{
        description: "",
        job: JobEnum.PAYSAN,
        value: 50
    }], [8]),
    [MeteosEnum.MANIFESTATION_RAVAL]: new Meteo(MeteosEnum.MANIFESTATION_RAVAL, "Fidèle à sa tâche — aussi morbide soit-elle, le Protecteur de Septange soutire le fluide vital de tout ce qui croît.", [{
        description: "",
        job: JobEnum.PECHEUR,
        value: -20
    }, {description: "", job: JobEnum.BUCHERON, value: -20}, {
        ...
        job: JobEnum.ALCHIMISTE,
        value: -20
    }, {description: "", job: JobEnum.PAYSAN, value: -20}], [9]),
    [MeteosEnum.QUIETUDE_MAIMANE]: new Meteo(MeteosEnum.QUIETUDE_MAIMANE, "Les frimas de l'hiver poussent les bûcherons à redoubler d'ardeur. A moins qu'il ne s'agisse de fournir la matière première nécessaire à la sculpture des totems rituels ?", [{
        description: "",
        job: JobEnum.BUCHERON,
        value: 60
    }], [10]),
    [MeteosEnum.GEL_BRUMAIRE]: new Meteo(MeteosEnum.GEL_BRUMAIRE, "Il y a gel et gel. Celui du Protecteur de Novamaire appartient à la seconde catégorie, en s'avérant de toute première catégorie.", [{
        description: "Les poissons se réfugient sous la glace, rendant leur capture particulièrement difficile.",
        job: JobEnum.PECHEUR,
        value: -45
    }, {
    ...
    [MeteosEnum.IRE_DJAUL]: new Meteo(MeteosEnum.IRE_DJAUL, "Qui a peur du Grand Méchant Djaul ? Tout le monde dans les environs. Sortir et risquer de le croiser ? Très peu pour vous !", [], [12])
}
